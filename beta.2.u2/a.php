...
echo'
<form method=post>
<select name=query><option>User Id</option><option>Last Name</option><option>First Name</option><option>Middle Name</option><option>Class</option></select><input type="textbox" name="txtSearch" placeholder="search here">';
...
echo"<div id=left>
<table border=1>"; 
echo"<td><input type=checkbox id='toggle' value='select' onClick='do_this()' /></td><td><span class='redh'>ID</span></td><td><span class='redh'>Last Name</span></td><td><span class='redh'>First Name</span></td><td><span class='redh'>Middle Name</span></td><td><span class='redh'>Class</span></td><td><span class='redh'>Time In</span></td><td><span class='redh'>Time Out</span></td>";

echo"<form method=post>";
echo"<input type=submit name=btnLogin value=Time-in class='log'>";
echo"&emsp;";
echo"<input type=submit name=btnLogout value=Time-out class='log'><br/><br/></div>";
...
{	
	...
	echo"<td><input type='submit' name=btn value='$id'></td><td>$lname</td><td>$fname</td><td>$mname</td><td>$class</td><td>$ti</td><td>$to</td></tr>";
	}
	}
	...
	echo"<td><input type='submit' name=btn value='$id'></td><td>$lname</td><td>$fname</td><td>$mname</td><td>$class</td><td>$ti</td><td>$to";
	}
	}
...
if(isset($_POST['btnLogin']))
{
	...
	if(!empty($_POST['chk'])) 
	{
		...
		{
		mysql_query("INSERT INTO User_Log(user_id,time_in,time_out,date) VALUES('$value','$time','$timeout','$date')");
		}
...
if(isset($_POST['btnLogout']))
{
	if(!empty($_POST['chk'])) 
	{
		foreach($_POST['chk'] as $value)
		{
		$result2=mysql_query("select time_in,time_out from User_Log where user_log_id=(select max(user_log_id) from User_Log where user_id='$value')");
...
echo"<div id=right>
<span class='redh'></span>
<br/><br/>
<table border=1 class=details>";
echo"<td><span class='redh'>Name</span></td><td><span class='redh'>Time in</span></td><td><span class='redh'>Time out</span></td>";
if(isset($_POST['btn']))
{
$value=$_POST['btn'];
$log=mysql_query("select User.user_id,User_Log.time_in,User_Log.time_out from User left join User_Log on User.user_id=User_Log.user_id where User_Log.user_id='$value' and User_Log.date='$date'");
